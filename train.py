...
def train_text_adapter(
    ...
    for epoch in range(start_epoch, text_epoch):
        logger.info(f"training text epoch {epoch + 1}:")
...
def train_image_adapter(
    ...
    for epoch in range(start_epoch, image_epoch):
        ...
        model_dict = {
            ...
            "image_scheduler": scheduler.state_dict(),
        }
...
def main():
    ...
    parser.add_argument("--decouple_weight", type=float, default=0) 
    parser.add_argument("--image_adapt_weight", type=float, default=0.1)
    parser.add_argument("--text_adapt_layers", type=int, nargs='+', default=[0,1,2])
    ...
    model = AdaptedCLIP(
        ...
        text_adapt_layers=args.text_adapt_layers,  
    ...
    logger.info(f"text dataset size: {len(text_dataset)}, text batch size: {args.text_batch_size}")
    logger.info(f"text iterations per epoch: {len(text_dataloader)}")
    logger.info(f"image dataset size: {len(image_dataset)}, image batch size: {args.image_batch_size}")
    logger.info(f"image iterations per epoch: {len(image_dataloader)}")
    # ========================================================
    # training
    if adapt_text:
